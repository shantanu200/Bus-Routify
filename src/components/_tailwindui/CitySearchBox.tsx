...
interface Props {
  setLocation: (value: Partial<ICity>) => void;
}

export default function CitySearchBox({ setLocation }: Props) {
  const [query, setQuery] = useState("");
  const [selectedLocation, setSelectedLocation] = useState<Partial<ICity>>();
  const [dbLocations, setDBLocations] = useState<ICity[]>();
  ...
  const debouncedFetchLocations = debounce(searchCities, 300);
  ...
  return (
    <Combobox as="div" value={selectedLocation} onChange={setSelectedLocation}>
      <div className="relative">
        <Combobox.Input
          className="w-full rounded-md border-0 bg-white py-2.5 pl-3 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300  sm:text-sm sm:leading-6"
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Start typing to search location"
          displayValue={(location: ICity) => location.city?.name}
        />
